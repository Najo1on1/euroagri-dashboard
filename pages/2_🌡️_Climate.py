...
# Try Plotly first, fall back to Matplotlib if anything goes wrong at render time.
PLOTLY_OK = True
try:
...
# -----------------------------
# Paths (prefer using src.data_paths if available)
# -----------------------------
try:
    from src import data_paths as P
    CLI_D = P.CLI_PARQ_D
    CLI_H = P.CLI_PARQ_H
except Exception:
    # Hard fallback if src.data_paths is not present
    WSL_D = Path("/mnt/d/Colab/Ecosystem/Deep RL and LLM/Agri")
    HOME  = Path("/home/najo1o11/euroagri-advisor")
    DATA_INTERIM_D = WSL_D / "data" / "interim"
    DATA_INTERIM_H = HOME  / "data" / "interim"
    CLI_D = DATA_INTERIM_D / "climate_windows.parquet"
    CLI_H = DATA_INTERIM_H / "climate_windows.parquet"
...
def load_parquet_with_fallback(primary: Path, fallback: Path) -> pd.DataFrame:
    try:
        ...
        st.warning(f"Failed to load parquet: {e}")
    return pd.DataFrame()
...
def aggregate_monthly(df: pd.DataFrame, all_years: bool, years: list[int] | None, metric_cols: list[str]) -> pd.DataFrame:
    ...
    if not all_years:
        ...
        # if years empty but not all_years, leave as-is (will just be empty if no match)
...
def plot_plotly(long_df: pd.DataFrame, metric: str, regions: list[str]):
    ...
    fig.update_layout(legend_title_text="Region", xaxis=dict(dtick=1))
...
# Load data
# -----------------------------
cli = canon(load_parquet_with_fallback(CLI_D, CLI_H))

with st.sidebar:
    st.header("Filters")
    if cli.empty:
        st.error("Climate file not found or empty.")
    # Regions
...
st.caption(f"Loaded climate rows: **{len(cli):,}** | regions: **{len(set(cli['region_iso'])) if 'region_iso' in cli.columns else 0}**")
...
st.download_button("⬇️ Download CSV (current view)", data=csv_bytes, file_name="climate_monthly_view.csv", mime="text/csv", use_container_width=True)

# -----------------------------
# Render charts
# -----------------------------
# Optional small multiples (precip as bars, temp as lines) can be approximated by plotting each metric separately.
cols = st.columns(1)
